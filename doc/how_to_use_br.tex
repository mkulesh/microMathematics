...
abre parênteses ''('' e então entre o
argumento. Este argumento também pode
ser uma fórmula, que contém quaisquer
...
a) elementos de arranjos podem ser
atribuídos usando um índice inferior
em vez de ''( )''. Para criar um índice
menor, coloque ''['' após o nome do
arranjo:
\begin{center}\begin{tabular}{ccc}
  $r_{0}  := 5$ &
  $r_{3}  := 6$ &
...
Qualquer elemento não atribuído é
definido por padrão como zero:
\begin{center}\begin{tabular}{cc}
  $idx := \left[ 0,\, 1 \,..\, 3 \right]$ &
  $r_{idx}  = \begin{bmatrix}5.0\\0.0\\-4.0\\6.0\\\end{bmatrix}$ \cr
\end{tabular}\end{center}

b) apenas um intervalo previamente
