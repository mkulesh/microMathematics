...
public class SVGParser extends DefaultHandler2
{
    ...
    // These static inner classes are only loaded/initialized when first used
    // and are thread safe
            ...
            throws SAXException
    {
        for (int i = 0; i < attributes.getLength(); i++)
        {
            String val = attributes.getValue(i).trim();
            if (val.length() == 0)
            { // The spec doesn't say how to handle empty
                // style attributes.
    ...
    private static Box parseViewBox(String val) throws SAXException
    {
        ...
        if (Float.isNaN(minX) || Float.isNaN(minY) || Float.isNaN(width)
                || Float.isNaN(height))
            throw new SAXException(
