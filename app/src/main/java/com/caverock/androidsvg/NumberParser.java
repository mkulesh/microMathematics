...
public class NumberParser
{
    ...
    public float parseNumber(String input, int startpos, int len)
    {
        ...
        if (significand != 0)
        {
            ...
            {
                // Some valid numbers can have an exponent greater than the max
                // (ie. < -38)
                // for a float. For example, significand=123, exponent=-40
                // If that's the case, we need to apply the exponent in two
                // steps.
                if (exponent < -38)
                {
                    // Long.MAX_VALUE is 19 digits, so taking 20 off the
                    // exponent should be enough.
                    f *= 1e-20;
                    exponent += 20;
                }
                // Do exponents < 0
